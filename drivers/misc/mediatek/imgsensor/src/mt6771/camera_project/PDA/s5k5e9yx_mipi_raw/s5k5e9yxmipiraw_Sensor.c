...
extern bool update_s5k5e9yx_eeprom(void);
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] = {
 ...
 //{ 2608, 1960,	  0,	0, 2608, 1960, 2608, 1960, 0000, 0000, 2608, 1960,	  8,	8, 2592, 1944}, // capture shawn modify for arcsoft request
...
static kal_uint32 open(void)
{
	...
	if(!(update_s5k5e9yx_eeprom()))
	{
		printk("Demon_eeprom update_eeprom error!");
	}
							 ...
							 UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	MSDK_SENSOR_REG_INFO_STRUCT *sensor_reg_data=(MSDK_SENSOR_REG_INFO_STRUCT *) feature_para;
       /* if(!feature_para || !feature_para_len){
           printk("[MIMI]main2 feature_control null pointer feature_para:%p  feature_para_len:%p\n", feature_para,feature_para_len);
           return -EFAULT;
        }*/
	...
	switch (feature_id) {
		...
		case SENSOR_FEATURE_SET_FRAMERATE:
			LOG_INF("[MIMI]2 current fps :%d\n", (UINT32)*feature_data_32);
			spin_lock(&imgsensor_drv_lock);
			imgsensor.current_fps = *feature_data_32; //misty isp5.0 need to change
			spin_unlock(&imgsensor_drv_lock);
			break;
		case SENSOR_FEATURE_SET_HDR:
			//LOG_INF("ihdr enable :%d\n", (BOOL)*feature_data_16);
			LOG_INF("[MIMI]2 Warning! Not Support IHDR Feature");
			spin_lock(&imgsensor_drv_lock);
